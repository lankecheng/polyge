...
public class Context {
    ...
    public init?(rootManagedObjectContext: NSManagedObjectContext, mainManagedObjectContext: NSManagedObjectContext) {
        if let coordinator = rootManagedObjectContext.persistentStoreCoordinator, let store = coordinator.persistentStores.first {
            ...
            if stackType != nil {
                ...
                stackContextOptions.fillEmptyOptions(true)
...
extension Context {
    ...
    internal func executeFetchRequest(fetchRequest: NSFetchRequest) throws -> [AnyObject]? {
        var objects :[AnyObject]?
        var error: NSError?
        if self.___background {
            // already in "performBlock"
            do{
                return try self.managedObjectContext.executeFetchRequest(fetchRequest)
    ...
    internal func executeAsynchronousFetchRequestWithFetchRequest(fetchRequest: NSFetchRequest, completionClosure: ([AnyObject]?, NSError?) -> Void) -> FetchAsyncHandler {
        ...
        moc.performBlock {
            if handler.cancelled {
                ...
                do{
                    handler.asynchronousFetchResult = try moc.executeRequest(asynchronousFetchRequest) as? NSAsynchronousFetchResult
                    handler.foolProgress.resignCurrent()
                    if handler.asynchronousFetchResult?.operationError != nil {
                        completionClosureCalled = true
    ...
    internal func executeBatchUpdateRequestWithEntityDescription(entityDescription: NSEntityDescription, propertiesToUpdate: [NSObject : AnyObject], predicate: NSPredicate, completionClosure: (Int, NSError?) -> Void) {
        ...
        moc.performBlock {
            do{
               let batchUpdateResult = try moc.executeRequest(batchUpdateRequest) as! NSBatchUpdateResult
