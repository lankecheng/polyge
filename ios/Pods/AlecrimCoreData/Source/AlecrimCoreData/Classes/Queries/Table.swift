...
extension Table {
    ...
    public func deleteEntity(entity: T) -> (success: Bool, error: NSError?) {
        do{
            let managedObjectInContext = try self.context.managedObjectContext.existingObjectWithID(entity.objectID)
    ...
    public func delete() -> (success: Bool, errors: [NSError]?) {
        ...
        if let objectIDs = self.executeFetchRequest(fetchRequest) as? [NSManagedObjectID] {
            for objectID in objectIDs {
                do{
                     let object = try self.context.managedObjectContext.existingObjectWithID(objectID)
...
extension Table {
    ...
    public func firstOrCreated(predicateClosure: (T.Type) -> NSComparisonPredicate) -> T {
        ...
        if let entity = self.filterBy(predicate).first() {
            return entity
        }
        else {
            let entity = self.createEntity()
...
extension Table {
    ...
    public func any(predicateClosure: (T.Type) -> NSPredicate) -> Bool {
        return self.filterBy(predicateClosure(T.self)).any()
    }
    
    public func count(predicateClosure: (T.Type) -> NSPredicate) -> Int {
        return self.filterBy(predicateClosure(T.self)).count()
    }
    
    public func filter(predicateClosure: (T.Type) -> NSPredicate) -> Self {
        return self.filterBy(predicateClosure(T.self))
    }
    
    public func first(predicateClosure: (T.Type) -> NSPredicate) -> T? {
        return self.filterBy(predicateClosure(T.self)).first()
    }
